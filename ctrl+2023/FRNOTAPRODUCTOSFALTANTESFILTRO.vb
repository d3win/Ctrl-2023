...
Public Class FRNOTAPRODUCTOSFALTANTESFILTRO
    Private Sub FRNOTAPRODUCTOSFALTANTESFILTRO_Load(sender As Object, e As EventArgs) Handles MyBase.Load

        'Dim idventa As String
        'conexionMysql.Open()
        'Dim Sql557 As String
        'Sql557 = "select max(idventa) from venta;"
        'Dim cmd557 As New MySqlCommand(Sql557, conexionMysql)
        'reader = cmd557.ExecuteReader()
        'reader.Read()
        ...
        'conexionMysql.Open()
        'Dim Sql55 As String
        'Sql55 = "select idcliente from venta where idventa=" & idventa & ";"
        ...
        'conexionMysql.Close()
        ''-----------------------------------
        Dim minimoproductos As Integer
        '-------------------
        'DATOS valor minimo producto para hacer pedido
        conexionMysql.Open()
        Dim Sqlm As String
        Sqlm = "select minimoproductos from datos_empresa;"
        Dim cmdm As New MySqlCommand(Sqlm, conexionMysql)
        reader = cmdm.ExecuteReader
        reader.Read()
        minimoproductos = reader.GetString(0).ToString()
        conexionMysql.Close()
        '------------------
        ...
        Dim ds As DataSet
        Dim Sql As String
        Sql = "SELECT producto.idproducto, producto.descripcion, producto.cantidad, proveedor.nombre_empresa FROM PRODUCTO,PROVEEDOR WHERE producto.CANTIDAD<=" & minimoproductos & " and proveedor.idproveedor=producto.idproveedor and proveedor.idproveedor=" & frmindex.idproveedorfiltro & ";"
        ...
        Dim rds As New ReportDataSource("dsproductosfaltantesfiltro", ds.Tables(0))
    ...
    Private Sub FRNOTAPRODUCTOSFALTANTESFILTRO_Load_1(sender As Object, e As EventArgs) Handles MyBase.Load
