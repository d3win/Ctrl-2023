...
Public Class FRMNotaVenta
    Private Sub FRMNotaVenta_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Sql = "select venta.idventa, (venta.cantidad) as cantidad1,(venta.total) as total1,venta.fecha,venta.hora, ventaind.actividad, (ventaind.cantidad)  as cantidad2,ventaind.costo,ventaind.idproducto, concat(cliente.nombre,' ',cliente.ap,' ', cliente.am)as cliente, (ventaind.cantidad * ventaind.costo) as totalind, ventaind.descripcion from venta,ventaind,cliente where venta.idventa = ventaind.idventa And ventaind.idventa = " & frmindex.indexidventa & " And cliente.idcliente = venta.idcliente;"
